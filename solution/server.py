...
class NumberGuessRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path == "/games":
            ...
            self.end_headers()
            self.wfile.write(json.dumps(list(self.game_instance.games.values())).encode("utf-8"))
    ...
    def do_PUT(self):
        if self.path.startswith("/games/"):
            ...
            self.game_instance.update_game(game_id, data)
            self.send_response(200)
            self.send_header("Content-type", "application/json")
            self.end_headers()
    ...
    def do_DELETE(self):
        if self.path.startswith("/games/"):
            game_id = int(self.path.split("/")[2])
            self.game_instance.delete_game(game_id)
